...
export class BookFormComponent implements OnInit, OnDestroy {
  ...
  private currentBookIdOnEdit!: number;
  ...
  onFormSubmit(): void {
    if (this.isEditFlowActive) {
      const bookToUpdate = { ...this.bookForm.value, id: this.currentBookIdOnEdit};
  ...
  private prepareCreateOrUpdateFlow() {
    const idFromUrlParam: number = this.activatedRoute.snapshot.params['id'];
    if (idFromUrlParam) {
      ...
      this.handleBookSelectedChanges(idFromUrlParam.toString());
    }
  }
